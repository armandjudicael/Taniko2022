...
public class DemandeurFormController implements Initializable {
    ...
    private void initTypeDemandeur(){
        typeDemandeur.getItems().addAll("Personne Morale","Personne Physique");
        typeDemandeur.getSelectionModel().selectedItemProperty().addListener((observableValue, s, t1) -> {
            if (t1.equals("Personne Morale")) moralPane.toFront();
            else physiquePane.toFront();
        });
    }
    ...
    private void initButton(){
        ...
        dmdNextBtn.setOnAction(event -> MainAffaireFormController.updateLabelAndShowPane(dmdNextBtn));
