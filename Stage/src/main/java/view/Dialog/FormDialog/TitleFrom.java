...
public class TitleFrom extends JFXDialog implements Initializable {
    ...
    private static Dossier exDossier;
    ...
    public static void setExAffaire(Dossier exDossier) {
        TitleFrom.exDossier = exDossier;
    }
        ...
        if (!numeroPropriete.getText().isEmpty() && !nomPropriete.getText().isEmpty()) {
            ...
            switch (operation){
                case CREATE_OPERATION:{
                    titre.setNumExAffaire(exDossier.getNumero());
...
//            int status = DbUtils.insertOnTerrainTitre(exAffaire.getTerrain(),TableName.ABOUTIR_TERRAIN_TITRE);
//            if (status == 1){
//                Platform.runLater(() -> {
