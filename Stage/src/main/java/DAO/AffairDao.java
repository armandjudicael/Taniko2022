...
public class AffairDao extends DAO<Affaire> {
    ...
    public int nombreDemandeAcquisition() {
        int nb = 0;
        String query = " SELECT COUNT(idAffaire) from affaire where typeAffaire ='ACQUISITION';";
        try (PreparedStatement ps = this.connection.prepareStatement(query)) {
            ResultSet resultSet = ps.executeQuery();
            while (resultSet.next())
                nb = resultSet.getInt("COUNT(id)");
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return nb;
    ...
    public int nombreTotalAffaire() {
        int nb = 0;
        String query = " SELECT COUNT(idAffaire) from affaire;";
        try (PreparedStatement ps = this.connection.prepareStatement(query)) {
            ResultSet resultSet = ps.executeQuery();
            while (resultSet.next())
                nb = resultSet.getInt("COUNT(id)");
