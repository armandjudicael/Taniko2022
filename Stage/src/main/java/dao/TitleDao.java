...
public class TitleDao extends Dao implements DaoHelper<Propriete>{

private static final String ALL_TITLE_COLUMN = "T.idTerrain AS IDENTIFIANT ," +
    ...
    private static final String EX_AFFAIRE = "(SELECT a.numAffaire from " +
            "propriete as t inner join affaire as a on t.idPropriete = a.terrainId where strcmp(a.situation,'REJETER') <> 0 )) AS EX_AFFAIRE";

    private static final String APPLICANT_FULLNAME = "(SELECT "+
            "DM.nomDmd " +
            "FROM demandeur_morale as DM inner join affaire a on DM.idDmd = a.demandeurId "+
            "inner join propriete t on a.terrainId = t.idPropriete ) AS APPLICANT_FULLNAME ";

    private static final String NUMERO_TITRE_MERE = "(SELECT t.numTitre from propriete t " +
            " inner join terrain_depend_propriete tdp on t.idPropriete = tdp.terrainParentId) AS NUM_TITRE_MERE";
    ...
    public ObservableList<Titre> getTitleBy(String dateCreation) {
        ...
        * */
        ...
        String query = "SELECT "+
                ...
                " terrain t inner join affaire a on t.idTerrain = a.terrainId  WHERE strcmp(a.situation,'REJETER') <> 0 ) is not null ;";
    ...
    public ObservableList<Titre> getAlltitle() {
        ObservableList<Titre> list = FXCollections.observableArrayList();
        String query = "SELECT " + FULL_TITLE_INFORMATION + " FROM terrain AS T ORDER BY T.idTerrain ;";
...
//    }
    ...
    @Override
    public int create(Propriete propriete) {
        return 0;
    }

    @Override
    public int delete(Propriete propriete) {
        return 0;
    }

    @Override
    public int update(Propriete propriete) {
        return 0;
    }
    ...
    @Override
    public Propriete finByNum(int num) {
        return null;
    }
