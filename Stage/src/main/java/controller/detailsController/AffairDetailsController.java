...
import static dao.DbUtils.executeQuery;
...
public class AffairDetailsController implements Initializable {
    ...
    @Override public void initialize(URL location, ResourceBundle resources){
        ...
        createTitleBtn.setOnAction(event ->{
            TableView<ConnexAffairForView> connexeTableView = ConnexeInfoController.getInstance().getConnexeTableView();
            ...
            MainService.getInstance().launch(new Task<Void>() {
                ...
                @Override protected Void call() throws Exception {
                     ...
                     *  > S'il en existe , alors on verifie le status de chaccun d'entre eux
                     *      - Si tous les status sont REJETER , alors on affiche le formulaire
                     *      - sinon on incite l'agent a rejeter les affaires connexes
                     *  > Sinon on affiche le formulaire de creation de titre
                     */
                    boolean terrainNonTitre = DaoFactory.getTerrainDao().checkTerrainAndTitre(propriete);
                      if (terrainNonTitre){
                          if (items!=null){
                              if (!items.isEmpty()){
                                  long count = items.stream().filter(connexAffairForView -> connexAffairForView.getStatus().equals(FolderStatus.REJECTED)).count();
                                  if (count==items.size()){
                                      Platform.runLater(() -> {
                          ...
                          Platform.runLater(() -> {
                              Alert alert = new Alert(Alert.AlertType.ERROR," Le propriete est déja titré ");
                              alert.show();
                          });
                      }
                    return null;
...
//        TitleFrom.setExAffaire(dossier);
//        Propriete propriete = dossier.getTerrain();
//        if (propriete !=null){
//            Titre titre = propriete.getTitreDependant();
//            if (titre != null) {
...
//        TitleFrom.getInstance().getPrenomTitulaire().setText(affaire.getDemandeur().getPrenom());
    ...
    private void editNomDemandeur(TextInputDialog dialog, Dossier dossier){
...
//            executeQuery(query);
    ...
    private void editPrenomDemandeur(TextInputDialog dialog, Dossier dossier){
...
//            executeQuery(query);
    ...
    private void editAdresseDemandeur(TextInputDialog dialog, Dossier dossier) {
...
//            executeQuery(query);
    ...
    private void editParcelleDemandeur(TextInputDialog dialog, Dossier dossier) {
...
//            executeQuery(query);
    ...
    private void editLotDemandeur(TextInputDialog dialog, Dossier dossier){
...
//            executeQuery(query);
    ...
    @FXML public void actionOnAffaire(ActionEvent actionEvent) {
        if (!EditorSecurity.getRemember()){
            FolderForView selectedItem = FolderViewController.getInstance().getTableView().getSelectionModel().getSelectedItem();
    ...
    public void launchAction(ActionEvent actionEvent){
        ...
        if (dossier != null) {
            editOperation(actionEvent, dossier);
        } else {
            if (AffairDetailsController.dossier != null)
                editOperation(actionEvent, AffairDetailsController.dossier);
        }
    }

    private void editNumeroAffaire(TextInputDialog dialog, Dossier dossier) {
        ...
        if (result.isPresent() && !result.equals(numAffaire)) {
            ...
            String query = " UPDATE affaire SET numAffaire = '" + result.get() + "' WHERE idAffaire = " + dossier.getId() + " ; ";
            executeQuery(query);
        }
    }

    private void editDateFormulation(Dossier dossier){
        ...
        datePicker.setValue(dossier.getDateDeFormulation().toLocalDate());
        ...
        if (result.isPresent()) {
            ...
            String query = " UPDATE affaire SET dateFormulation = '" + result.get() + "' WHERE idAffaire = " + dossier.getId() + " ; ";
            executeQuery(query);
        }
    }

    private void editTypeDemande(Dossier dossier) {
        ...
        if (choice.isPresent()) {
            ...
            String query = " UPDATE affaire SET typeAffaire = '" + choice.get() + "' WHERE idAffaire = " + dossier.getId() + ";";
            executeQuery(query);
        }
    }

    private void editStatus(Dossier dossier){
        String type[] = {"En cours", "Suspendu", "Terminé", "Réjété"};
        ChoiceDialog<String> dialog = new ChoiceDialog<>(type[0], type);
        Optional<String> choice = dialog.showAndWait();
        FolderStatus value = FolderStatus.RUNNING;
        if (choice.isPresent()) {
            String choix = choice.get();
            if (choix.equals("En cours")) {
                value = FolderStatus.RUNNING;
                statusLabel.setGraphic(new ImageView(FolderViewController.getRunningImg()));
                statusLabel.setText(" En cours ");
            } else if (choix.equals("Suspendu")) {
                statusLabel.setGraphic(new ImageView(FolderViewController.getSuspendImg()));
                statusLabel.setText(" Suspendu ");
                value = FolderStatus.SUSPEND;
            } else if (choix.equals("Terminé")) {
                statusLabel.setGraphic(new ImageView(FolderViewController.getFinishedImg()));
                statusLabel.setText("Terminé");
                value = FolderStatus.SUCCEED;
            } else if (choix.equals("Réjété")) {
                statusLabel.setGraphic(new ImageView(FolderViewController.getRejectImg()));
                statusLabel.setText("Réjété");
                value = FolderStatus.REJECTED;
            }
            String query = " UPDATE affaire SET situation = '" + Dossier.affaireStatus2String(value) + "' WHERE idAffaire = " + dossier.getId() + " ; ";
            executeQuery(query);
        }
    }

    private void editSuperficie(Dossier dossier){
        if (superficieDialog == null){
            ...
            String value[] = dossier.getTerrain().getSuperficie().toString().split("-");
            if (value.length == 3) {
                ...
                String[] sup = dossier.getTerrain().getSuperficie().toString().split(" ");
                if (sup[1].equals("Ha"))
            ...
            superficieDialog.setResultConverter(new Callback<ButtonType, String>() {
                ...
                public String call(ButtonType param){
                    return "";
//                    return MainAffaireForm.getSuperficie(hectareInput, areInput, centiAreInput);
                }
            });
        }
        ...
        if (result.isPresent() && !result.get().isEmpty()) {
            ...
            String query = " UPDATE terrain SET  superficie ='"+newSuperficie+"' WHERE idTerrain =" + dossier.getTerrain().getIdPropriete()+ ";";
            executeQuery(query);
        }

    }

    private void editSise(TextInputDialog dialog, Dossier dossier) {
        ...
        if (result.isPresent() && !result.equals(numAffaire) && !result.get().isEmpty()) {
            ...
            String query = " UPDATE terrain SET quartier = '" + result.get() + "' WHERE idTerrain = " + dossier.getTerrain().getIdPropriete() + " ; ";
            executeQuery(query);
        }
    }

    private void editParcelleTerrain(TextInputDialog dialog, Dossier dossier){
        ...
        if (result.isPresent() && !result.equals(numAffaire)) {
            ...
            String query = " UPDATE terrain SET parcelle = '" + result.get() + "' WHERE idTerrain = " + dossier.getTerrain().getIdPropriete() + " ; ";
            executeQuery(query);
        }

    }

    private void editDistrict(TextInputDialog dialog, Dossier dossier) {
        ...
        if (result.isPresent() && !result.equals(district)) {
            ...
            String query = " UPDATE terrain SET district = '" + result.get() + "' WHERE idTerrain = " + dossier.getTerrain().getIdPropriete() + " ; ";
            executeQuery(query);
        }
    }

    private void editOperation(ActionEvent actionEvent, Dossier dossier) {
        TextInputDialog dialog = new TextInputDialog();
        if (actionEvent.getSource() == edtNameDmdBtn) { editNomDemandeur(dialog, dossier); }
        else if (actionEvent.getSource() == edtFirstNameBtn) editPrenomDemandeur(dialog, dossier);
        else if (actionEvent.getSource() == edtAdressBtn) editAdresseDemandeur(dialog, dossier);
        else if (actionEvent.getSource() == edtParcelleBtn) editParcelleDemandeur(dialog, dossier);
        else if (actionEvent.getSource() == edtLotBtn) editLotDemandeur(dialog, dossier);
        else if (actionEvent.getSource() == edtNumBtn) editNumeroAffaire(dialog, dossier);
        else if (actionEvent.getSource() == edtDateFormBtn) editDateFormulation(dossier);
        else if (actionEvent.getSource() == edtTypeDmdBtn) editTypeDemande(dossier);
        else if (actionEvent.getSource() == editStatusBtn) editStatus(dossier);
        else if (actionEvent.getSource() == edtSiseBtn) editSise(dialog, dossier);
        else if (actionEvent.getSource() == edtParcelleTerrainBtn) editParcelleTerrain(dialog, dossier);
        else if (actionEvent.getSource() == edtCommuneBtn) editCommune(dialog, dossier);
        else if (actionEvent.getSource() == edtDistrictBtn) editDistrict(dialog, dossier);
        else if (actionEvent.getSource() == edtRegionBtn) editRegion(dialog, dossier);
        else if (actionEvent.getSource() == edtSuperficieBtn) editSuperficie(dossier);
        else if (actionEvent.getSource() == edtTitreMereBtn) {
            TitreMereEditDialog.getInstance().show();
        }
    }

    private void editCommune(TextInputDialog dialog, Dossier dossier) {
        ...
        if (result.isPresent() && !result.equals(commune)) {
            ...
            String query = " UPDATE terrain SET commune = '" + result.get() + "' WHERE idTerrain = " + dossier.getId() + " ; ";
            executeQuery(query);
        }
    }
    private void editRegion(TextInputDialog dialog, Dossier dossier) {
        ...
        if (result.isPresent() && !result.equals(region)) {
            ...
            String query = " UPDATE terrain SET region = '" + result.get() + "' WHERE idTerrain = " + dossier.getId() + " ; ";
            executeQuery(query);
        }
    }
    ...
    public static Dossier getAffaire() {
        return dossier;
    }
    public static void setAffaire(Dossier dossier) {
        AffairDetailsController.dossier = dossier;
    }
    public static AffairDetailsController getInstance() {
        return detailsController;
    }
    private static Dossier dossier = null;
