        ...
        Task<Void> task = new Task<Void>() {
            ...
            protected Void call() throws Exception {
                ...
                if (!InitializeApp.getAffaires().isEmpty()) {
                    Optional<FolderForView> first = InitializeApp.getAffaires().stream().filter(affairForView -> affairForView.getNumero().equals(num)).findFirst();
                    if (first.isPresent())
                        dossier = first.get();
                } else {
                    dossier = DaoFactory.getDossierDao().finByNum(num);
                }
                FolderViewController.getInstance().show(dossier);
